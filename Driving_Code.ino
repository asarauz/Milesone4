...
int time1 = 0;                                            // encoder pulses per motor revolution
               ...
               if (motorsEnabled) {                                            // run motors only if enabled
                     ...
                     switch(driveIndex) {                                      // cycle through drive states
                        case 0: // Stop
                           ...
                           encoder[1].pos = 0;           // stop right motor
                        ...
                        case 2: // Drive backward — motors spin in opposite directions as they are opposed by 180 degrees
                           ...
                           if(encoder[0].pos >= 2192){ // right motor reverse (opposite dir from left)
                           driveIndex = 3;
                           encoder[0].pos = 0; 
                           time1 = millis();}
                        ...
                        case 3: // Turn right (clockwise) — motors spin in same direction
                           setMotor(1, driveSpeed, cIN1Chan[0], cIN2Chan[0]); // left motor reverse 
                           setMotor(1, driveSpeed, cIN1Chan[1], cIN2Chan[1]);  // right motor forward (opposite dir from right)
                             
                           if(encoder[0].pos >= 2100){ // right motor reverse (opposite dir from left)
                           encoder[0].pos = 0; 
                           driveIndex = 4;
                           time1 = millis();}                                   // next state: turn left                                    // next state: stop
                           break;
                        case 4: // Turn right (clockwise) — motors spin in same direction
                           ...
                           if(encoder[0].pos <= -4384){
                             ...
                             setMotor(0, driveSpeed, cIN1Chan[1], cIN2Chan[1]);                                               // clear left encoder
                          driveIndex =5;}                                  // next state: drive backward
                                               // next state: turn left                                    // next state: stop
                           break;
                     }
